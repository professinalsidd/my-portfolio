...
const CertificatesComp = () => {
  ...
  const isAllVisible = visibleProjects >= certificates.length;

  const handleToggleProjects = () => {
    setVisibleProjects(isAllVisible ? 5 : certificates.length);
  };

  return (
    <React.Fragment>
      <Box sx={[LAYOUT.columnEnd, { width: "90%", mt: 2 }]}>
        <Typography sx={styles.heading}>Certificates</Typography>
      </Box>
      {/* Projects List */}
      <Box sx={styles.content}>
        {certificates.slice(0, visibleProjects).map((project, index) => (
